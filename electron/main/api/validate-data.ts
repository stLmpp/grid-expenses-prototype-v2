...
function formatValidationErrors(validationErrors: ValidationError[]): string[] {
  return validationErrors.reduce(
    (acc, item) => [...acc, ...formatValidationError(item)],
    [] as string[]
  );
}
...
export async function validateData(
  instanceOrInstances: AnyObject | AnyObject[]
): Promise<string[]> {
  const instances = coerceArray(instanceOrInstances);
  const errors = await Promise.all(
    instances.map((instance) => validate(instance, { whitelist: true }))
  ).then((nestedErrors) => nestedErrors.flat());
