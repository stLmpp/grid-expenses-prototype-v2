...
export class ModuleResolver {
  private constructor(
    ...
    private readonly moduleMetadata: ModuleOptions
  ) {
  ...
  private _getAllOptionsFromModule(
    ...
    const controllers = [
      ...
      ...moduleOptionsChildren.controllers,
    ];
    return this._moduleSet.set(module, { providers, controllers }).get(module)!;
  }

  private _getAllOptionsFromModules(
    modules: Array<Class<any> | ModuleWithProviders>
  ): ModuleWithoutImports {
  ...
  static create(injector: Injector, module: Class<any>): ModuleResolver {
    ...
    if (!metadata) {
      throw new Error(
        `Module "${module.name}" not configured. Did you forget to put the @Module decorator?`
      );
    }
